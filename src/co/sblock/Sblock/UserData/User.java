...
public class User {
	...
	/** Classpect */
	protected UserClass classType;
	protected UserAspect aspect;
	protected MediumPlanet mPlanet;
	protected DreamPlanet dPlanet;

	/** The Player's tower number */
	protected byte tower;

	/** Locations to teleport Players to when conditions are met */
	protected Location previousLocation, serverDisableTeleport;

	/** Programs installed to the player's computer */
	protected HashSet<Integer> programs;

	/** Checks made while the Player is logged in, not saved. */
	protected boolean isServer, allowFlight;

	/** The UUIDs of the Player's server and client players. */
	protected UUID server, client;

	/** A map of the Effects applied to the Player and their strength. */
	protected HashMap<PassiveEffect, Integer> passiveEffects;
