 ...
 * registered.<p>
 * 
 * About the command system: Command handlers are placed into a class implementing the CommandListener interface, and
 * must be registered in the module through registerCommands(CommandListener).Each command handler 
 ...
 * 
 * 
 * Each command (As of now at least) must still be defined in plugin.yml, and its usage message and permissions
...
public @interface SblockCommand {
     ...
     * 
     * Indicates that the last argument in this command method should receive all of the remaining words the player types,
