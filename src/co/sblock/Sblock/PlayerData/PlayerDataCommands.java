 ...
 * 
 * For more information on how the command system works, please see {@link co.sblock.Sblock.SblockCommand}
 * 
 * @author FireNG
 * 
 */
public class PlayerDataCommands implements CommandListener {
    ...
    public boolean profile(CommandSender sender, String playerToLookup) {
        SblockPlayer player = PlayerManager.getPlayerManager().getPlayer(playerToLookup);
        if (player == null)
            sender.sendMessage(ChatColor.YELLOW + "Player not found.");
        else {
            String message = 
                PROFILE_COLOR + "-----------------------------------------\n" + 
             ChatColor.YELLOW + playerToLookup + ": " + player.getClassType().getDisplayName() + " of " + player.getAspect().getDisplayName() + "\n" +
                PROFILE_COLOR + "-----------------------------------------\n" + 
                                "Dream planet: " + ChatColor.YELLOW + player.getDPlanet().getDisplayName() + "\n" + 
                PROFILE_COLOR + "Medium planet: " + ChatColor.YELLOW + player.getDPlanet().getDisplayName() + "\n" + 
