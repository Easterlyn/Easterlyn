...
public class EasterlynVillagers extends JavaPlugin {
  ...
  private void adjustMerchant(final Merchant merchant) {
    if (merchant instanceof PersistentDataHolder) {
      ...
      int storedVersion =
          dataHolder
              ...
              .getOrDefault(versionKey, PersistentDataType.INTEGER, 0);
    ...
    if (merchant instanceof PersistentDataHolder) {
      ((PersistentDataHolder) merchant)
          .getPersistentDataContainer()
          .set(versionKey, PersistentDataType.INTEGER, VERSION);
    }
  }

  private @Nullable MerchantRecipe adjustRecipe(final MerchantRecipe recipe) {
    ...
    ItemStack input2 =
        recipe.getIngredients().size() > 1 ? recipe.getIngredients().get(1) : ItemUtil.AIR;
    ...
    try {
      ...
      ReportableEvent.call(
          String.format("Error adjusting trade: %s + %s -> %s", input1, input2, recipe.getResult()),
          e,
          5);
      e.printStackTrace();
      return null;
    ...
    if (adjusted == null
        && input1.getType() != Material.WRITTEN_BOOK
      ...
      ReportableEvent.call(
          String.format(
              "Unable to adjust trade: %s + %s -> %s", input1, input2, recipe.getResult()));
    }

    return adjusted;
  }

  private @Nullable MerchantRecipe adjustRecipe(
      @NotNull ItemStack input1,
      @NotNull ItemStack input2,
      @NotNull ItemStack result,
      final int uses,
      final int maxUses,
      final boolean giveExp,
      final int villagerExperience) {
    if (result.getType() == Material.EXPERIENCE_BOTTLE
        || input1.getType() == Material.WRITTEN_BOOK) {
      return null;
    }
    if (CurrencyType.isCurrency(input1)
        && (input2.getType() == Material.AIR || CurrencyType.isCurrency(input2))
        && !CurrencyType.isCurrency(result)) {
      // TODO: Does not support value > 64EB
      //  (e.g. item worth 80 EB will be unpurchasable instead of 64 and 16 EB)
      // Purchase result - deal is not supposed to be good.
      // Use overpriced rate for worth of result.
      double resultCost =
          NumberUtil.multiplySafe(EconomyUtil.getWorth(result), EconomyUtil.OVERPRICED_RATE);
      ...
      double remainder = resultCost - CurrencyType.valueOf(input1);
      MerchantRecipe recipe =
          new MerchantRecipe(result, uses, maxUses, giveExp, villagerExperience, 0.0F);
    ...
    if (!CurrencyType.isCurrency(input1)
        && CurrencyType.isCurrency(input2)
        && !CurrencyType.isCurrency(result)) {
      ...
      double inputCost =
          NumberUtil.multiplySafe(EconomyUtil.getWorth(result), EconomyUtil.UNDERPRICED_RATE);
  ...
  private ItemStack getSingleMoneyStack(final double worth, final RoundingMode mode) {
    ...
    for (int i = currency.ordinal() + 1; amount > 1 && i < currencies.length; ++i) {
      ...
      amount > 64
          ...
          || amount > currencyMultiplier * 4
              && Math.abs(roundRemainder - currencyMultiplier / 2) < currencyMultiplier / 6) {
        // Set new values
  ...
  private enum CurrencyType {
    ...
    static int valueOf(final ItemStack currencyItem) {
      for (CurrencyType currencyType : CurrencyType.values()) {
        if (currencyType.getCurrencyItem().isSimilar(currencyItem)) {
