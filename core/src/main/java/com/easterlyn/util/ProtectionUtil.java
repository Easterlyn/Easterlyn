...
public class ProtectionUtil {
	...
	static {
		new Reflections("com.easterlyn.util.protection").getSubTypesOf(ProtectionHook.class).stream()
	...
	public static boolean canUseButtonsAt(Player player, Location location) {
		return HOOKS.stream().anyMatch(hook -> hook.isHookUsable() && hook.canUseButtonsAt(player, location));
	}

	public static boolean canOpenChestsAt(Player player, Location location) {
		return HOOKS.stream().anyMatch(hook -> hook.isHookUsable() && hook.canOpenChestsAt(player, location));
	}

	public static boolean canBuildAt(Player player, Location location) {
		return HOOKS.stream().anyMatch(hook -> hook.isHookUsable() && hook.canBuildAt(player, location));
	}
