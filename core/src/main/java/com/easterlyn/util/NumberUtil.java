...
public class NumberUtil {
   ...
   *
   * <p>Throws an IllegalArgumentException if the BigInteger provided is negative or if the base is
   ...
   * @throws NumberFormatException if the String given contains characters which are not Roman
   *     numerals
   */
  ...
  public static long parseDuration(String input) throws NumberFormatException {
    ...
    if (matcher.find()) {
      ...
      if (matcher.group(2) != null) {
        ...
        time =
            Math.addExact(time, Math.multiplyExact(31536000000L, Long.parseLong(matcher.group(2))));
      }
      if (matcher.group(4) != null) {
        // Months: 30d * 24h * 60m * 60s * 1000ms
        time =
            Math.addExact(time, Math.multiplyExact(2592000000L, Long.parseLong(matcher.group(4))));
      }
      if (matcher.group(6) != null) {
        // Weeks: 7d * 24h * 60m * 60s * 1000ms
        time =
            Math.addExact(time, Math.multiplyExact(604800000L, Long.parseLong(matcher.group(6))));
      }
