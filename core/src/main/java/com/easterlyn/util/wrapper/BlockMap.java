...
public class BlockMap<T> {
  ...
  private final Map<String, TreeMap<Integer, TreeMap<Integer, Map<Integer, T>>>> serverMap =
      new HashMap<>();

  public @Nullable T put(@NotNull Block block, @Nullable T value) {
    TreeMap<Integer, TreeMap<Integer, Map<Integer, T>>> worldMap =
        serverMap.computeIfAbsent(block.getWorld().getName(), k -> new TreeMap<>());
    TreeMap<Integer, Map<Integer, T>> blockXMap =
        worldMap.computeIfAbsent(block.getX(), k -> new TreeMap<>());
    Map<Integer, T> blockZMap =
        blockXMap.computeIfAbsent(block.getZ(), (blockZ) -> new HashMap<>());
  ...
  public @NotNull Collection<T> get(@NotNull String world, int chunkX, int chunkZ) {
    ...
    SortedMap<Integer, TreeMap<Integer, Map<Integer, T>>> chunkXSubMap =
        worldMap.subMap(blockXMin, blockXMin + 16);
    ...
    for (Map.Entry<Integer, TreeMap<Integer, Map<Integer, T>>> blockXEntry :
        chunkXSubMap.entrySet()) {
      SortedMap<Integer, Map<Integer, T>> chunkZSubMap =
          blockXEntry.getValue().subMap(blockZMin, blockZMin + 16);
  ...
  public @NotNull Collection<T> remove(@NotNull String world, int chunkX, int chunkZ) {
    ...
    for (Iterator<TreeMap<Integer, Map<Integer, T>>> blockXIterator =
            chunkXSubMap.values().iterator();
        blockXIterator.hasNext(); ) {
      TreeMap<Integer, Map<Integer, T>> blockXValue = blockXIterator.next();
      SortedMap<Integer, Map<Integer, T>> chunkZSubMap =
          blockXValue.subMap(blockZMin, blockZMin + 16);

      for (Iterator<Map<Integer, T>> blockZIterator = chunkZSubMap.values().iterator();
          blockZIterator.hasNext(); ) {
   ...
   */
  public @NotNull Collection<Map.Entry<Block, T>> entrySet() {
