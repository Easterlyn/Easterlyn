...
public class Dublexor extends Machine {
    ...
    if (exampleRecipe == null) {
      ...
      GenericUtil.consumeAs(
          ...
          itemMeta -> {
            ...
            List<String> lore =
                Arrays.asList(
                    ChatColor.WHITE + "Displays dublecation cost",
      ...
      GenericUtil.consumeAs(
          ...
          itemMeta -> {
            ...
            List<String> lore =
                Collections.singletonList(ChatColor.WHITE + "Dublecate your items.");
            itemMeta.setLore(lore);
  ...
  private Pair<ItemStack, Integer> unCaptcha(ItemStack potentialCaptcha) {
    ...
    RegisteredServiceProvider<EasterlynCaptchas> registration =
        getMachines().getServer().getServicesManager().getRegistration(EasterlynCaptchas.class);
    ...
    while (EasterlynCaptchas.isUsedCaptcha(potentialCaptcha)) {
      ...
      multiplier =
          Math.multiplyExact(multiplier, Math.max(1, Math.abs(potentialCaptcha.getAmount())));
  ...
  private void displayTrade(
      ...
      @NotNull MerchantInventory open,
      @NotNull ItemStack input,
      @NotNull ItemStack expCost,
      @NotNull ItemStack result,
      boolean fullUpdate) {
    List<MerchantRecipe> recipes = new ArrayList<>(open.getMerchant().getRecipes());

    // Check if selected recipe is correct for input
    if (open.getSelectedRecipeIndex() > 0
        && input.equals(recipes.get(open.getSelectedRecipeIndex()).getIngredients().get(0))) {
  ...
  private void setSlots(
      @NotNull Player player,
      @NotNull Inventory open,
      @Nullable ItemStack expCost,
