...
public class EffectRocketJump extends Effect {
  ...
  public void applyEffect(@NotNull LivingEntity entity, int level, Event event) {
    ...
    if (entity.getMetadata(getName()).stream()
        .anyMatch(value -> value.asLong() > System.currentTimeMillis())) {
      return;
    }
    entity.removeMetadata(getName(), getPlugin());
    entity.setMetadata(
        getName(), new FixedMetadataValue(getPlugin(), System.currentTimeMillis() + 1000));

    entity.setFallDistance(0);
    entity.getWorld().spawnParticle(Particle.EXPLOSION_HUGE, entity.getLocation(), 1);

    final Firework firework =
        (Firework) entity.getWorld().spawnEntity(entity.getLocation(), EntityType.FIREWORK);
