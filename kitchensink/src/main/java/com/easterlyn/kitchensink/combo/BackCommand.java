...
public class BackCommand extends BaseCommand implements Listener {
  ...
  public void back(
    ...
    if (!other && user.getStorage().getLong(BACK_COOLDOWN) >= System.currentTimeMillis()) {
      ...
      issuer.sendInfo(
          MessageKey.of("sink.module.back.error.cooldown"),
          "{value}",
          dateFormat.format(
              new Date(user.getStorage().getLong(BACK_COOLDOWN) - System.currentTimeMillis())));
      return;
    }
    ...
    new BossBarTimer(
            ...
            () -> {
              if (player.teleport(back)) {
                if (other) {
                  issuer.sendInfo(
                      MessageKey.of("sink.module.back.success_other"),
                      "{target}",
                      player.getName());
                } else {
